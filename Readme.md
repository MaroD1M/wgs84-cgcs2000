...
- **移除旧代码**：已删除 `old/` 历史目录，避免误用过时代码。
- **关闭调试模式**：本地启动默认 `debug=False`，避免调试信息暴露。
- **上传大小限制**：服务端限制单次上传文件最大 **10MB**。
- **批量行数限制**：单次批量转换最多 **20000 行**，防止异常大文件导致内存压力。
- **请求参数校验**：新增 `sourceType`、`decimalPlaces`、模板下载参数校验，减少异常输入风险。
- **导出安全**：保留并强化临时文件名与路径校验，防止路径遍历。
