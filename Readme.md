...
### v1.1 发布说明
- 移除历史旧代码目录 `old/`，降低维护歧义与误用风险。
- 新增服务端安全校验：`sourceType`、`decimalPlaces`、模板下载参数格式校验。
- 增加上传安全阈值：单文件最大 10MB，批量最多 20000 行。
- 保持并强化导出路径安全策略，避免非法文件名/路径访问。
- 新增基础接口测试 `tests/test_api_smoke.py`，覆盖核心成功与错误路径。
